//
//  BatteryInformationPacketDecoderTests.swift
//  Niimbot Printer
//
...
final class BatteryInformationPacketDecoderTests: XCTestCase {
    
    func testDecode_SuccessfulDecoding() throws {
        let inputData: [UInt8] = [0x01]
        let packet = Packet(requestCode: .RESPONSE_GET_INFO_BATTERY, data: inputData)
        ...
        expectation(forNotification: .App.batteryInformation,
                    object: nil,
                    handler: handler)
        
        let decoder = BatteryInformationPacketDecoder()
    ...
    func testDecode_UnsuccessfulDecoding() throws {
        ...
        let e = expectation(forNotification: .App.batteryInformation,
                            object: nil,
                            handler: nil)
        e.isInverted = true
